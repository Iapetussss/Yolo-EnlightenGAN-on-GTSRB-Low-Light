#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
生成GTSRB数据集的三列检测结果对比图
包含：Baseline / 传统增强 / EnlightenGAN 三种方法
"""

import cv2
import numpy as np
from pathlib import Path
import matplotlib.pyplot as plt
from ultralytics import YOLO
import random

# 设置中文字体
plt.rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei', 'Arial Unicode MS', 'DejaVu Sans']
plt.rcParams['axes.unicode_minus'] = False

def draw_detections(image, results, conf_threshold=0.25, color=(0, 255, 0)):
    """在图像上绘制检测结果"""
    img_with_detections = image.copy()
    
    if results and len(results) > 0 and results[0].boxes is not None:
        boxes = results[0].boxes
        for box in boxes:
            # 获取检测框信息
            conf = float(box.conf[0])
            if conf < conf_threshold:
                continue
                
            # 获取坐标
            x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
            x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
            
            # 绘制检测框
            cv2.rectangle(img_with_detections, (x1, y1), (x2, y2), color, 2)
            
            # 绘制标签
            label = f'{conf:.2f}'
            label_size, _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
            cv2.rectangle(img_with_detections, (x1, y1 - label_size[1] - 5), 
                        (x1 + label_size[0], y1), color, -1)
            cv2.putText(img_with_detections, label, (x1, y1 - 5),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
    
    return img_with_detections

def main():
    print("="*70)
    print("  生成GTSRB三列检测结果对比图")
    print("  (Baseline / 传统增强 / EnlightenGAN)")
    print("="*70)
    
    # 路径配置 - GTSRB数据集
    original_dir = Path("data/baseline_lowlight_dataset/images/test")
    traditional_dir = Path("data/traditional_enhanced/images/test")
    enlightengan_dir = Path("data/enhanced/images/test")
    
    # 模型路径
    baseline_model_path = Path("experiments/exp1_baseline/run8/weights/best.pt")
    traditional_model_path = Path("experiments/exp2_traditional/run/weights/best.pt")
    enlightengan_model_path = Path("experiments/exp3_enlightengan/run/weights/best.pt")
    
    output_dir = Path("results/gtsrb_triple_detection")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # 检查目录
    if not original_dir.exists():
        print(f"❌ 原始图像目录不存在: {original_dir}")
        return
    
    if not traditional_dir.exists():
        print(f"❌ 传统增强图像目录不存在: {traditional_dir}")
        return
    
    if not enlightengan_dir.exists():
        print(f"❌ EnlightenGAN增强图像目录不存在: {enlightengan_dir}")
        return
    
    # 检查模型文件
    models_to_load = {}
    if baseline_model_path.exists():
        models_to_load['baseline'] = baseline_model_path
    else:
        print(f"⚠️  Baseline模型不存在: {baseline_model_path}")
    
    if traditional_model_path.exists():
        models_to_load['traditional'] = traditional_model_path
    else:
        print(f"⚠️  传统增强模型不存在: {traditional_model_path}")
    
    if enlightengan_model_path.exists():
        models_to_load['enlightengan'] = enlightengan_model_path
    else:
        print(f"⚠️  EnlightenGAN模型不存在: {enlightengan_model_path}")
    
    if len(models_to_load) == 0:
        print("\n❌ 没有找到任何模型文件")
        return
    
    # 加载模型
    print("\n📦 加载模型...")
    models = {}
    for name, path in models_to_load.items():
        try:
            models[name] = YOLO(str(path))
            print(f"  ✓ {name} 模型加载成功")
        except Exception as e:
            print(f"  ✗ {name} 模型加载失败: {e}")
    
    if len(models) == 0:
        print("\n❌ 没有成功加载任何模型")
        return
    
    # 获取所有图像
    all_images = sorted(original_dir.glob("*.png")) + sorted(original_dir.glob("*.jpg"))
    
    if len(all_images) == 0:
        print("❌ 没有找到图像")
        return
    
    print(f"📊 找到 {len(all_images)} 张测试图像")
    
    # 评估所有图像，选择最佳样本
    print(f"\n🔍 评估所有图像，选择最佳对比样本...")
    
    candidate_scores = []
    
    # 评估每张图像
    for img_path in all_images[:100]:  # 限制评估前100张，加快速度
        try:
            original_img = cv2.imread(str(img_path))
            if original_img is None:
                continue
            
            # 检查图像尺寸（选择较大的图像）
            h, w = original_img.shape[:2]
            if h < 200 or w < 200:  # 跳过太小的图像
                continue
            
            # 使用baseline模型快速检测
            if 'baseline' in models:
                results = models['baseline'](original_img, verbose=False)
                det_count = len(results[0].boxes) if results and len(results) > 0 and results[0].boxes is not None else 0
                
                # 计算检测框的平均置信度
                if det_count > 0:
                    confidences = [float(box.conf[0]) for box in results[0].boxes]
                    avg_conf = np.mean(confidences)
                else:
                    avg_conf = 0
                
                # 评分：检测数量适中(3-6个) + 置信度较高 + 图像较大
                if 3 <= det_count <= 6 and avg_conf > 0.5:
                    score = det_count * 10 + avg_conf * 100 + (h + w) / 10
                    candidate_scores.append((score, img_path, det_count, avg_conf, h, w))
        except:
            continue
    
    # 按分数排序，选择最好的
    if len(candidate_scores) > 0:
        candidate_scores.sort(reverse=True, key=lambda x: x[0])
        best_score, best_image, det_count, avg_conf, h, w = candidate_scores[0]
        print(f"  ✓ 选择最佳图像: {best_image.name}")
        print(f"    检测数量: {det_count}, 平均置信度: {avg_conf:.2f}, 尺寸: {w}x{h}")
    else:
        # 如果没找到，放宽条件再找
        print("  ⚠️  未找到理想样本，放宽条件重新搜索...")
        for img_path in all_images[:200]:
            try:
                original_img = cv2.imread(str(img_path))
                if original_img is None:
                    continue
                h, w = original_img.shape[:2]
                if h < 150 or w < 150:
                    continue
                
                if 'baseline' in models:
                    results = models['baseline'](original_img, verbose=False)
                    det_count = len(results[0].boxes) if results and len(results) > 0 and results[0].boxes is not None else 0
                    if det_count >= 2:  # 至少2个检测
                        best_image = img_path
                        print(f"  ✓ 选择图像: {best_image.name} (检测数量: {det_count}, 尺寸: {w}x{h})")
                        break
            except:
                continue
        
        if best_image is None:
            best_image = all_images[0]
            print(f"  ⚠️  使用第一张图像: {best_image.name}")
    
    print(f"📂 输出目录: {output_dir}\n")
    
    try:
        # 读取原始图像
        original_img = cv2.imread(str(best_image))
        if original_img is None:
            print("❌ 原始图像读取失败")
            return
        
        original_rgb = cv2.cvtColor(original_img, cv2.COLOR_BGR2RGB)
        
        # 读取传统增强图像
        traditional_path = traditional_dir / best_image.name
        if not traditional_path.exists():
            print(f"❌ 传统增强图像不存在: {traditional_path}")
            return
        
        traditional_img = cv2.imread(str(traditional_path))
        if traditional_img is None:
            print("❌ 传统增强图像读取失败")
            return
        
        traditional_rgb = cv2.cvtColor(traditional_img, cv2.COLOR_BGR2RGB)
        
        # 读取EnlightenGAN增强图像
        gan_path = enlightengan_dir / best_image.name
        if not gan_path.exists():
            print(f"❌ EnlightenGAN增强图像不存在: {gan_path}")
            return
        
        enlightengan_img = cv2.imread(str(gan_path))
        if enlightengan_img is None:
            print("❌ EnlightenGAN增强图像读取失败")
            return
        
        enlightengan_rgb = cv2.cvtColor(enlightengan_img, cv2.COLOR_BGR2RGB)
        
        # 使用对应模型检测
        # (a) Baseline检测原始图像
        if 'baseline' in models:
            baseline_results = models['baseline'](original_img, verbose=False)
            original_with_det = draw_detections(original_rgb, baseline_results, color=(0, 255, 0))
        else:
            original_with_det = original_rgb
        
        baseline_det_count = len(baseline_results[0].boxes) if 'baseline' in models and baseline_results and len(baseline_results) > 0 and baseline_results[0].boxes is not None else 0
        
        # (b) 传统增强模型检测传统增强图像
        if 'traditional' in models:
            traditional_results = models['traditional'](traditional_img, verbose=False)
            traditional_with_det = draw_detections(traditional_rgb, traditional_results, color=(0, 200, 255))
        elif 'baseline' in models:
            traditional_results = models['baseline'](traditional_img, verbose=False)
            traditional_with_det = draw_detections(traditional_rgb, traditional_results, color=(0, 200, 255))
        else:
            traditional_with_det = traditional_rgb
        
        traditional_det_count = len(traditional_results[0].boxes) if ('traditional' in models or 'baseline' in models) and traditional_results and len(traditional_results) > 0 and traditional_results[0].boxes is not None else 0
        
        # (c) EnlightenGAN模型检测增强图像
        if 'enlightengan' in models:
            enlightengan_results = models['enlightengan'](enlightengan_img, verbose=False)
            enlightengan_with_det = draw_detections(enlightengan_rgb, enlightengan_results, color=(255, 0, 0))
        elif 'baseline' in models:
            enlightengan_results = models['baseline'](enlightengan_img, verbose=False)
            enlightengan_with_det = draw_detections(enlightengan_rgb, enlightengan_results, color=(255, 0, 0))
        else:
            enlightengan_with_det = enlightengan_rgb
        
        enlightengan_det_count = len(enlightengan_results[0].boxes) if ('enlightengan' in models or 'baseline' in models) and enlightengan_results and len(enlightengan_results) > 0 and enlightengan_results[0].boxes is not None else 0
        
        # 创建三列对比图（1行3列，适合Word，增大尺寸）
        # 根据图像尺寸调整figsize
        img_height = original_img.shape[0]
        img_width = original_img.shape[1]
        aspect_ratio = img_width / img_height
        
        # 计算合适的显示尺寸（确保图像清晰）
        display_width = max(8, min(12, img_width / 100))  # 每列宽度
        display_height = max(6, min(10, img_height / 100))  # 高度
        
        fig, axes = plt.subplots(1, 3, figsize=(display_width * 3, display_height))
        
        # (a) Baseline检测结果
        axes[0].imshow(original_with_det)
        axes[0].set_title('(a) Baseline\n(mAP@0.5: 70.40%)', 
                        fontsize=14, fontweight='bold', pad=15)
        axes[0].axis('off')
        axes[0].text(0.5, -0.08, f'检测数量: {baseline_det_count}', 
                   transform=axes[0].transAxes, ha='center', fontsize=12, color='green', fontweight='bold')
        
        # (b) 传统增强检测结果
        axes[1].imshow(traditional_with_det)
        axes[1].set_title('(b) 传统增强\n(Retinex+CLAHE+Gamma)\n(mAP@0.5: 70.08%)', 
                        fontsize=14, fontweight='bold', color='orange', pad=15)
        axes[1].axis('off')
        axes[1].text(0.5, -0.08, f'检测数量: {traditional_det_count}', 
                   transform=axes[1].transAxes, ha='center', fontsize=12, color='orange', fontweight='bold')
        
        # (c) EnlightenGAN检测结果
        axes[2].imshow(enlightengan_with_det)
        axes[2].set_title('(c) EnlightenGAN\n(mAP@0.5: 39.66%)', 
                        fontsize=14, fontweight='bold', color='red', pad=15)
        axes[2].axis('off')
        axes[2].text(0.5, -0.08, f'检测数量: {enlightengan_det_count}', 
                   transform=axes[2].transAxes, ha='center', fontsize=12, color='red', fontweight='bold')
        
        # 添加总标题
        plt.suptitle('GTSRB低光照场景检测结果对比', 
                    fontsize=16, fontweight='bold', y=0.98)
        
        # 添加说明文字
        fig.text(0.5, 0.02, 
                'Baseline和传统增强方法表现接近，EnlightenGAN因resize导致小目标细节丢失，性能显著下降',
                ha='center', fontsize=10, style='italic')
        
        plt.tight_layout()
        plt.subplots_adjust(bottom=0.08, top=0.92)
        
        # 保存图像（超高分辨率，确保清晰）
        output_path = output_dir / f"gtsrb_triple_detection_{best_image.stem}.png"
        plt.savefig(output_path, dpi=300, bbox_inches='tight', facecolor='white', pad_inches=0.1)
        print(f"✅ 三列对比图已保存: {output_path}")
        
        # 也保存为PDF格式（矢量图，更适合Word）
        output_path_pdf = output_dir / f"gtsrb_triple_detection_{best_image.stem}.pdf"
        plt.savefig(output_path_pdf, bbox_inches='tight', facecolor='white', pad_inches=0.1)
        print(f"✅ 三列对比图已保存: {output_path_pdf}")
        
        # 额外保存一个高分辨率版本（如果图像很大）
        if img_width > 500 or img_height > 500:
            output_path_hd = output_dir / f"gtsrb_triple_detection_{best_image.stem}_HD.png"
            plt.savefig(output_path_hd, dpi=400, bbox_inches='tight', facecolor='white', pad_inches=0.1)
            print(f"✅ 高清版本已保存: {output_path_hd}")
        
        plt.close()
        
        print(f"\n{'='*70}")
        print(f"✅ 三列对比图生成完成！")
        print(f"{'='*70}\n")
        print(f"图像: {best_image.name}")
        print(f"检测数量: Baseline={baseline_det_count}, 传统增强={traditional_det_count}, EnlightenGAN={enlightengan_det_count}")
        print(f"\n📂 结果保存在: {output_dir}/")
        print(f"\n💡 适合插入Word文档，建议使用PDF格式（矢量图，清晰度更高）")
        
    except Exception as e:
        print(f"❌ 错误: {e}")
        import traceback
        traceback.print_exc()

if __name__ == '__main__':
    main()

